...
namespace lab7
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int id = 35900;
            bool exit = false;
            Console.WriteLine("Choose 1 for example, 2 for entering data");
        ...
        static string ProcessFindFamilyDoctorPatients(AdditHashtable table, string doc)
        {
            ...
            if(patients.patient.Count == 0)
                return $"Doc.{doctor} do not have any patient yet";
            else{
                Console.WriteLine($"Doc.{doctor}:");
                Patient[] array = new Patient[patients.patient.Count];
                patients.patient.CopyTo(array);
        ...
        static void Control(Hashtable table, ref int id, AdditHashtable addTable)
        {
            ...
            Console.WriteLine(ProcessRemoving(table, "Lolo,Selec", addTable));
            System.Threading.Thread.Sleep(2000);
            Console.WriteLine("Command:Print");
            table.Print();
            System.Threading.Thread.Sleep(2000);
            Console.WriteLine("Command:FindFamilyDoctorPatients\nMarinina");
        ...
        static string DocName(AdditHashtable addTable, Key key, Hashtable table)
        {
            while(true)
            {
                ...
                if(doc.doctor != null && doc.patient.Count == 5)
                {
